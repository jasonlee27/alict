...
class AbstractTest(ABC):
    ...
    def _extract_examples_per_testcase(
        ...
        if not only_include_fail:
            idxs = [i for i in np.argsort(expect_results)]
        else:
    ...
    def print(self, xs, preds, confs, expect_results, labels=None, meta=None, format_example_fn=None, nsamples=3, logger=None):
        ...
        if not result:
            return
        idxs, iters, _expect_results = result
        if logger is not None:
            _print = logger.print
        else:
            _print = print
        # end if
        for i, (x, pred, conf, label, meta) in enumerate(iters):
            _print(format_example_fn(x, pred, conf, label, meta, _expect_results[i]))
    ...
    def run(self, predict_and_confidence_fn, overwrite=False, verbose=True, n=None, seed=None, logger=None):
        """Runs test
        ...
        if verbose:
            _print('Predicting %d examples' % len(examples))
    ...
    def pass_idxs(self):
        self._check_results()
        return np.where(self.results.passed == True)[0]
    ...
    def print_stats(self, logger=None):
        ...
        if logger is not None:
            ...
            _print = print
        # end if
        _print('Test cases:      %d' % stats.testcases)
        if 'testcases_run' in stats:
            _print('Test cases run:  %d' % stats.testcases_run)
        if 'after_filtering' in stats:
            _print('After filtering: %d (%.1f%%)' % (stats.after_filtering, stats.after_filtering_rate))
        if 'fails' in stats:
            _print('Fails (rate):    %d (%.1f%%)' % (stats.fails, stats.fail_rate))
    ...
    def summary(self, n=3, print_fn=None, format_example_fn=None, n_per_testcase=3, logger=None):
        ...
        def default_format_example(x, pred, conf, *args, **kwargs):
            ...
            label, meta, expect_result = args[0], args[1], args[2]
            pred_res = "FAIL" if expect_result!=True else "PASS"
            binary = False
            if softmax:
                if conf.shape[0] == 2:
                    conf = conf[1]
                    return 'DATA::%s::%.1f::%s::%s::%s' % (pred_res, conf, pred, label, str(x))
                elif conf.shape[0] <= 4:
                    confs = ' '.join(['%.1f' % c for c in conf])
                    return 'DATA::%s::%s::%s::%s::%s' % (pred_res, confs, pred, label, str(x))
                else:
                    conf = conf[pred]
                    return 'DATA::%s::%.1f::%s::%s::%s' % (pred_res, conf, pred, label, str(x))
            else:
                return 'DATA::%s::[]::%s::%s::%s' % (pred_res, pred, label, str(x))

        if format_example_fn is None:
            format_example_fn = default_format_example
            
        passs = self.pass_idxs()
        if passs.shape[0] != 0:
            _print()
            ...
            for p in passs:
                ...
                print_fn(self.data[d_idx], self.results.preds[d_idx],
                         ...
                         label, meta, format_example_fn, nsamples=n_per_testcase, logger=logger)
