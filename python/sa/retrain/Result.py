...
class Result:
    ...
    @classmethod
    def get_results_per_requirement_from_string(cls, model_result_str, task):
        p = re.compile(
            f"(.*?)?\nTest cases\:      (\d*)\nFails (rate)\:    (\d*) \(\d*\.?\d*\%\)\n\nExample passes\:\n(.*?)?\nExample fails\:\n(.*?)?\n\n",
            re.DOTALL
        )
        model_results_per_reqs = [m.strip() for m in p.findall(model_result_str)]
