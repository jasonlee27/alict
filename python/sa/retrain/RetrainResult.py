...
class RetrainResult:
    ...
    @classmethod
    def get_checklist_results_per_requirement_from_string(cls, model_result_str):
        pattern = f"(.*?)?\nTest cases\:      (\d*)\nFails (rate)\:    (\d*) \(\d*\.?\d*\%\)\n\nExample passes\:\n(.*?)?\nExample fails\:\n(.*?)?\n\n"
        p = re.compile(pattern, re.DOTALL)
        model_results_per_reqs = list()
        for m in p.findall(model_result_str):
            model_results_per_reqs.append({
                'req': m.group(1),
                'pass': cls.get_pass_sents_from_model_string(m.group(5)),
                'fail': cls.get_fail_sents_from_model_string(m.group(6))
            })
        return model_results_per_reqs
    ...
    def read_orig_checklist_result(cls, task, dataset_name, selection_method, retrained_model_name):
        ...
        model_result_str = cls.parse_model_results(result_str, retrained_model_name)
        result_reqs = cls.get_checklist_results_per_requirement_from_string(model_result_str)
    ...
    def read_orig_ours_result(cls, task, dataset_name, selection_method, retrained_model_name):
        ...
        model_result_str = cls.parse_model_results(result_str, retrained_model_name)
        result_reqs = cls.get_ours_results_per_requirement_from_string(model_result_str)
    ...
    @classmethod
    def read_retrained_checklist_result(cls, retrained_model_name):
        ...
        result_str = cls.read_result_file(checklist_orig_result_file)
        model_result_str = cls.parse_model_results(result_str, retrained_model_name, is_retrained_model=True)
        result_reqs = cls.get_checklist_results_per_requirement_from_string(model_result_str)
        checklist_testsuite_dict = {retrained_model_name: list()}
        for r in result_reqs:
            if r['req'] in cls.CHECKLIST_LC_LIST:
    ...
    @classmethod
    def read_retrained_ours_result(cls, retrained_model_name):
    ...
    def analyze(cls, task, dataset_name, selection_method, retrained_model_name):
        ...
        _retrained_model_name = f"{task}_{dataset_name}_{model_name}"
