...
readonly PYTHON_DIR="${_DIR}/python/qqp"
...
function gen_requirements() {
        ...
        (cd ${_DIR}
         python -m python.qqp.main --run requirement
        )
}
...
function eval_models(){
        ...
        (cd ${_DIR}
         # time python -m python.qqp.main --run testmodel --test_baseline # evaluating models on checklist testcases
         time python -m python.qqp.main --run testmodel # evaluating models on our generated testcases
        )
}
...
function eval_retrained_models(){
        # evaluate NLP models with generated testsuites
        (cd ${_DIR}
         python -m python.qqp.main --run testmodel --local_model_name checklist-textattack-bert-base-uncased-SST-2
        )
}
